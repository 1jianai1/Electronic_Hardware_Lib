...
/*引脚初始化*/
#if I2C_HARDWARE==1
//todo I2c的初始化在cubemx中完成, 你也可以自定义
//I2C_HandleTypeDef hi2c1;

#else
void OLED_W_SCL(uint8_t x){
    HAL_GPIO_WritePin(OLED_I2C_SCL_Port, OLED_I2C_SCL_Pin, x);
}
void OLED_W_SDA(uint8_t x){
    HAL_GPIO_WritePin(OLED_I2C_SDA_Port,OLED_I2C_SDA_Pin, x);
}
#endif
...
void OLED_ShowFloat(uint8_t Line,uint8_t Column,float Number,uint8_t Length,uint8_t point_len){
    uint8_t i;
    uint32_t m = OLED_Pow(10,point_len);

    uint16_t point = (int)(Number*m)%m;
    uint16_t Zheng = (int)Number;
    OLED_ShowSignedNum(Line,Column,Zheng,Length);
    OLED_ShowChar(Line,Column+Length+1,'.');
    OLED_ShowNum(Line,Column+Length+2,point,point_len);
}

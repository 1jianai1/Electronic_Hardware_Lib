...
//求偏移量
// 手动实现计算从低位开始连续 0 的个数
static inline uint8_t count_trailing_zeros(uint32_t value) {
    uint8_t count = 0;
...
#define IIC_SCL_OFFSET (uint8_t)(count_trailing_zeros(IIC_SCL))
#define IIC_SDA_OFFSET (uint8_t)(count_trailing_zeros(IIC_SDA))

//SCL-->PB8
//SDA-->PB9
#define SDA_IN()  do { \
    IIC_SDA_PORT->MODER &= ~(3U << (IIC_SDA_OFFSET * 2)); \
} while(0)

// 配置 SDA 引脚为输出模式
#define SDA_OUT() do { \
    IIC_SDA_PORT->MODER &= ~(3U << (IIC_SDA_OFFSET * 2)); \
    IIC_SDA_PORT->MODER |= 1U << (IIC_SDA_OFFSET * 2); \
} while(0)

//IO操作函数	 
#define IIC_SCL_H    IIC_SCL_PORT->BSRR = IIC_SCL           //SCL
#define IIC_SCL_L     IIC_SCL_PORT->BSRR = IIC_SCL<< 16U    //SCL
#define IIC_SDA_H    IIC_SDA_PORT->BSRR = IIC_SDA           //SDA
#define IIC_SDA_L    IIC_SDA_PORT->BSRR = IIC_SDA<< 16U     //SDA
...
{			
  ...
  GPIO_InitStructure.Pin = IIC_SCL;
  GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_OD; //选择模式
  GPIO_InitStructure.Pull = GPIO_NOPULL; //开漏输出类型
